...
import { parseImportFile, type GpxSport, type ImportDraft } from "./newActivity.import";
...
export function mountNewActivity(root: HTMLElement) {
  ...
  async function handleGpxInput(input: HTMLInputElement) {
    ...
    for (const file of files) {
      ...
      try {
        const result = await parseImportFile(file, DETECT_BYTES);
        if (!result.ok) {
