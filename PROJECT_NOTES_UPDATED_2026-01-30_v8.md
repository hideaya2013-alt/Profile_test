...
- 目的：直近ターンをUIに残し、会話の流れを維持する（DB保存はしない）
- 保存方針（現段階）
  - **DBへは保存しない**
  - **finalText（APIへ投げた本文）はどこにも保存しない**（トークン/機微/肥大化対策）
  - UI表示用として、`userText` と `replyText` だけを **軽量キャッシュ** する
    - 既定：**直近5ターン**（= user/assistant の5往復）
    - 1メッセージ上限：例 800 chars（超えたら末尾をトリム）
  - 方式：
    - メモリ（画面が生きている間の表示）＋ `localStorage`（リロード後も復元）
    - 「画面遷移で消えてもよい」は維持しつつ、戻ってきたら復元できる設計でOK
    - クリア導線（任意）：`Clear chat` ボタン or `/?dev` から clear（後で追加可）

#### フィールド（UI用・最小）
- id: string
- role: "user" | "assistant"
- content: string  // userText or replyText（finalTextは禁止）
- createdAt: string（ISO）
...
##### 実装責務（破綻しない分離：固定）
##### Chat UIログ（キャッシュ方針：固定）
- 目的：使い勝手のため、直近の会話を少しだけ見えるようにする
- **保存する**：`userText` / `replyText`（直近5ターン）
- **保存しない**：`finalText`（context pack + user input を結合したAPI送信本文）
- 実装案：
  - `src/screens/triCoachChat.ts` の state に `messages: ChatMessage[]`
  - 送信成功で user/assistant を push → 5ターン超過は先頭から drop
  - `localStorage` に `tricoach.chat.recent` として保存（最大5ターン）
  - mount時に `localStorage` から復元（存在しなければ空）
...
- POST /v1/chat
  ...
  - Response: { replyText: string, requestId: string, proposal?: null | object }
...
#### 起動メモ（Windows / VS Code）
- VS Code の Python Interpreter が **仮想環境（例：trimenu_env）** になっているか必ず確認する（baseだと import missing になりやすい）
- `uvicorn.exe` 直叩きが環境ポリシーでブロックされる場合があるため、起動は **python -m** を推奨：
...
### 6.4 Proposal（plan提案）導入ロードマップ（stub後に着手）
1) **まずは形だけ**：`POST /v1/chat` の stubレスポンスに `proposal: null` を追加（UIは変えない）
2) **カード枠だけ出す**：`proposal !== null` のときだけ Chat画面に「提案カード枠」を表示（中身はダミーでOK）
3) **中身を詰める**：proposalに「最大7日分のplan」を載せ、Menu画面へ Apply で反映（自動反映はしない）

> 注：proposalは `replyText` と別物として扱う（データ型が違うため）。
