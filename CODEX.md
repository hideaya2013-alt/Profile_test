...
## 7. Safety Rails v1（変更安全柵 / 不変）

目的：局所UI修正での「白画面」「静かに崩壊」を未然に防ぐ。

### 7.1 変更範囲（Scope）を固定する
- 1タスク=1ファイル（原則）。複数ファイル変更が必要なら、理由と影響範囲を冒頭に明記する。
- 1タスク=1論点（例：Saveボタン表示だけ）。UI改善＋状態整理＋CSS刷新などの同時実施は禁止。

### 7.2 リネーム禁止（破壊的変更の抑止）
- 既存の定数名・関数名・data属性名はリネームしない（値の変更は可）。
- 既存識別子を削除/置換する場合は、必ずファイル内検索で参照が残っていないことを確認する。
  - 例：SAVE_* を削るなら、triCoachChat.ts 内に SAVE_ が残っていないこと。

### 7.3 className全面上書き禁止（不可視化事故の抑止）
- `element.className = "..."` の全面置換で、基底レイアウトを破壊しない。
- 原則：baseクラスは維持し、差分は追加・条件分岐で切替える。

### 7.4 例外握りつぶし禁止（静かな白画面の抑止）
- `catch {}` の空catchは禁止。例外は console.error するか、UIに fault を出す。
- 画面初期化/描画/更新の中核（mount/render/bind/update）で例外を握りつぶさない。

### 7.5 最小スモークテスト（完了条件）*エージェントモードの時
変更後、必ず以下を満たしてから完了とする：
- TriCoach画面を開いて真っ白にならない（DOMが表示される）
- DevTools Console にエラーが出ていない（0件）
- 変更したUIの操作が1回通る（例：Save→Saved→戻る）

### 7.6 差分提示ルール
- 変更後は「変更ファイル一覧」「差分の要点」「手動テスト手順」を必ず出力する。
