...
- 新フィールド追加後も、**既存DB（古いレコード）にはキーが存在しない**前提で扱う
- `load*()` 等の **load系で必ず補完・正規化**してから UI に渡す
- UIは補完後の値を表示し、保存時に以後の永続化形式を確定する（undefined を残さない）
- DB上で「値が存在しない」はエラーではない（欠損は許容する）

### 1.3 DB設計軸（共通認識：不変）
- DB I/O は `src/db.ts` に集約し、画面は DB を唯一の正として扱う
- 履歴の最小単位は **Activity**（GPX / Manual を分離しない）
- 値が存在しない場合は **`null` を保存する**（DBに `undefined` を残さない）
- sRPE は **常に取得・保存・表示・計算対象**（Profile の on/off はメニュー生成時の考慮トリガに限定）
